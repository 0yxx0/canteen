...
public class OrderInfoDaoImpl implements IOrderInfoDao{
	...
	@Override
	public List<OrderInfo> week() {
		DBHelper db = new DBHelper();
		String sql = "SELECT DATE_FORMAT(odate,'%Y-%m-%d') as odate,sum(price) price from" + 
				" orderinfo where status=0  group by DATE_FORMAT(odate,'%Y-%m-%d')";
